...
export class PolicyComponent implements OnInit {
  ...
  postResponses() {
    ...
    for(let i = 0; i < this.questions.length; i++){
      ...
      this.postReportService.postUserResponse(this.reports[0].responseId, this.questions[i].questionId, this.policyForm.get('answer'+(i+1))?.value).subscribe();
    }
  }
  ...
  onSubmit(): void {
    ...
    if(this.nextClicked) {
      ...
      this.postResponses();
      this.router.navigate([ './survey-outro' ])
    }
    /**Backwards */
    else{ 
      console.log(this.policyForm.value);
