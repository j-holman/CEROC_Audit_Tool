...
@Component({
  ...
  styleUrls: ['./continuity.component.css']
})
export class ContinuityComponent implements OnInit {
  continuityForm!: FormGroup;
  nextClicked = false;
  questions: Question[] = [];
  errorMessage = '';
  currentUser: any;
  ...
  initializeForm(): void {
    this.continuityForm = this.fb.group({
      answer1: '',
      answer2: '',
      answer3: '',
      answer4: '',
  ...
  postResponses() {
    ...
    for(let i = 0; i < this.questions.length; i++){
      ...
      this.postReportService.postUserResponse(this.reports[0].responseId, this.questions[i].questionId, this.continuityForm.get('answer'+(i+1))?.value).subscribe();
    }
  }
  ...
  onSubmit(): void {
    /**Forwards */
    if(this.nextClicked) {
      console.log(this.continuityForm.value);
      this.postResponses();
      this.router.navigate([ './incident-response' ])
    }
    /**Backwards */
    else{ 
      console.log(this.continuityForm.value);
