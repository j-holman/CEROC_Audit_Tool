...
@Component({
  selector: 'app-phys-security',
  templateUrl: './phys-security.component.html',
  styleUrls: ['./phys-security.component.css']
})
export class PhysSecurityComponent implements OnInit {
  physSecurityForm!: FormGroup;
  nextClicked = false;
  questions: Question[] = [];
  errorMessage = '';
  currentUser: any;
  ...
  initializeForm(): void {
    this.physSecurityForm = this.fb.group({
      answer1: '',
      answer2: '',
      answer3: ''
    })
  ...
  postResponses() {
    ...
    for(let i = 0; i < this.questions.length; i++){
      ...
      this.postReportService.postUserResponse(this.reports[0].responseId, this.questions[i].questionId, this.physSecurityForm.get('answer'+(i+1))?.value).subscribe();
    }
  }
  ...
  onSubmit(): void {
    /**Forwards */
    if(this.nextClicked) {
      console.log(this.physSecurityForm.value);
      this.postResponses();
