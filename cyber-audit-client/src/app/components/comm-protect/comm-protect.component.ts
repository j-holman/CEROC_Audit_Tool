...
@Component({
  selector: 'app-comm-protect',
  templateUrl: './comm-protect.component.html',
  styleUrls: ['./comm-protect.component.css']
})
export class CommProtectComponent implements OnInit {
  commProtectForm!: FormGroup;
  nextClicked = false;
  questions: Question[] = [];
  errorMessage = '';
  currentUser: any;
  ...
  initializeForm(): void {
    this.commProtectForm = this.fb.group({
      answer1: '',
      answer2: '',
      answer3: ''
    })
  ...
  postResponses() {
    ...
    for(let i = 0; i < this.questions.length; i++){
      ...
      this.postReportService.postUserResponse(this.reports[0].responseId, this.questions[i].questionId, this.commProtectForm.get('answer'+(i+1))?.value).subscribe();
    }
  }
  ...
  onSubmit(): void {
    /**Forwards */
    if(this.nextClicked) {
      console.log(this.commProtectForm.value);
      this.postResponses();
      this.router.navigate([ './config-mgmt' ])
    }
    /**Backwards */
    else{ 
      console.log(this.commProtectForm.value);
