...
@Component({
  ...
  templateUrl: './config-mgmt.component.html',
  styleUrls: ['./config-mgmt.component.css']
})
export class ConfigMgmtComponent implements OnInit {
  configMgmtForm!: FormGroup;
  nextClicked = false;
  questions: Question[] = [];
  errorMessage = '';
  currentUser: any;
  ...
  initializeForm(): void {
    this.configMgmtForm = this.fb.group({
      answer1: '',
      answer2: '',
      answer3: '',
      answer4: '',
  ...
  postResponses() {
    ...
    for(let i = 0; i < this.questions.length; i++){
      ...
      this.postReportService.postUserResponse(this.reports[0].responseId, this.questions[i].questionId, this.configMgmtForm.get('answer'+(i+1))?.value).subscribe();
    }
  }
  ...
  onSubmit(): void {
    /**Forwards */
    if(this.nextClicked) {
      console.log(this.configMgmtForm.value);
      this.postResponses();
