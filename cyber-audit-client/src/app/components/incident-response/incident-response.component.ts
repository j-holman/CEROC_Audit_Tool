...
export class IncidentResponseComponent implements OnInit {
  incidentResponseForm!: FormGroup;
  nextClicked = false;
  questions: Question[] = [];
  errorMessage = '';
  currentUser: any;
  ...
  initializeForm(): void {
    this.incidentResponseForm = this.fb.group({
      answer1: '',
      answer2: '', 
      answer3: '',
      answer4: ''
  ...
  postResponses() {
    ...
    for(let i = 0; i < this.questions.length; i++){
      ...
      this.postReportService.postUserResponse(this.reports[0].responseId, this.questions[i].questionId, this.incidentResponseForm.get('answer'+(i+1))?.value).subscribe();
    }
  }
  ...
  onSubmit(): void {
    /**Forwards */
    if(this.nextClicked) {
      console.log(this.incidentResponseForm.value);
      this.postResponses();
      this.router.navigate([ './monitor-malware' ])
    }
    /**Backwards */
    else{ 
      console.log(this.incidentResponseForm.value);
